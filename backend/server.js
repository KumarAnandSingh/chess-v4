...
// Fallback health endpoint for simple health checks
...
function startRoomGame(roomCode) {
  try {
    ...
    // Update room status to playing to prevent duplicate starts
    ...
    if (startResult.success) {
      // Emit game start to room with properly structured data
      const gameData = {
        gameId: game.gameId,
        gameState: {
          position: startResult.gameState.position,
          clock: startResult.gameState.clock,
          players: startResult.gameState.players, // Keep as array for frontend transformation
          status: startResult.gameState.status,
          currentTurn: startResult.gameState.currentTurn,
          moveHistory: startResult.gameState.moveHistory || []
        },
        room: room
      };

      io.to(`room_${roomCode}`).emit('game_started', gameData);
...
function createMatchmakingGame(playerIds, settings) {
  try {
    ...
    if (startResult.success) {
      ...
      io.to(gameRoom).emit('game_found', gameData);
